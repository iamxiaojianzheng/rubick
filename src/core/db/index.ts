...
export default class DB {
  ...
  public async dumpDb(config: { url: string; username: string; password: string }): Promise<void> {
    const webdavClient = new WebDavOP(config);
    webdavClient.createWriteStream(this.pouchDB);
  }

  public async importDb(config: { url: string; username: string; password: string }): Promise<void> {
    const webdavClient = new WebDavOP(config);
