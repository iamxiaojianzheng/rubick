...
interface Rubick {
  ...
  /** 设置子输入框（回调接收输入文本） */
  setSubInput(onChange: (param: { text: string }) => void, placeholder?: string, isFocus?: boolean): boolean;
  /** 移除子输入框 */
  removeSubInput(): void;
  /** 设置子输入框内容 */
  ...
  createBrowserWindow(url: string, options: BrowserWindowCreateOptions, callback?: () => void): RubickBrowserWindow;
}

// 全局声明：允许在window上直接访问rubick
declare global {
  let rubick: Rubick;
  let LOCAL_PLUGINS: any;
  let OP_CONFIG: any;
}
